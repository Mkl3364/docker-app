...
const Form = () => {
    ...
    const onSubmit: SubmitHandler<ValidationSchema> = async (data) => {
        const accountResponse = await fetch('http://localhost:3001/users', {
            method: 'POST',
            body: JSON.stringify({
        ...
        <form className="px-8 pt-6 pb-8 mb-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mb-4 md:flex md:justify-between">
            ...
            <div className="md:ml-2">
              ...
              />
              {errors.confirmPassword && (<p className="text-xs italic text-red-500 mt-2">{errors.confirmPassword.message}</p>)}
            </div>
          </div>
          <div className="mb-4">
